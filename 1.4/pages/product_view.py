...
top_products = (
    product_df.groupby("Product Name")
    .agg({
        "Sales": "sum",
        "Category": "first",
        "Sub-Category": "first",
...
categories = product_df["Category"].dropna()
categories = [c for c in categories.unique() if isinstance(c, str)]
categories = ["Tất cả"] + sorted(categories)
selected_category = st.selectbox("Lọc theo Category", categories)
if selected_category != "Tất cả":
    subcats = product_df[product_df["Category"] == selected_category]["Sub-Category"]
else:
    subcats = product_df["Sub-Category"]

subcats = subcats.dropna()
subcats = [s for s in subcats.unique() if isinstance(s, str)]
subcats = ["Tất cả"] + sorted(subcats)
selected_subcat = st.selectbox("Lọc theo Sub-Category", subcats)

# Lọc sản phẩm
all_products = product_df.groupby("Product Name").agg({
...
if not all_products.empty:
    ...
    def add_to_cart():
        ...
        row = product_df[product_df["Product Name"]==selected_product].iloc[0]
    ...
    def buy_now():
        product_rows = cart_data[cart_data["Product Name"] == selected_product]
        ...
        # --- Tạo ngày ---
        order_date = pd.to_datetime(datetime.now().date())  # datetime.date -> datetime64
        ship_date = order_date + pd.to_timedelta(random.randint(1, 4), unit="d")
        ...
        if not existed_order.empty:
            # Cập nhật đơn cũ
            idx = existed_order.index[0]
            df_existing.loc[idx, "Order Date"] = order_date
            df_existing.loc[idx, "Ship Date"] = ship_date
            ...
            row["Order ID"] = f"BUY-{datetime.now().strftime('%Y%m%d%H%M%S%f')}"

            # Thêm vào DataFrame
            df_existing = pd.concat([df_existing, pd.DataFrame([row])], ignore_index=True)

        # --- Lưu CSV giữ datetime ---
        df_existing.to_csv(csv_file, index=False, encoding="ISO-8859-1", date_format="%Y-%m-%d")
    ...
    def get_dl_suggestions(username, cart_data, model, top_k=8):
        ...
        all_products = torch.tensor(product_enc.transform(cart_data["Product Name"].unique()))
        scores = []
        ...
        suggestions = cart_data[cart_data["Product Name"].isin(top_products)].drop_duplicates("Product Name")
        return suggestions
