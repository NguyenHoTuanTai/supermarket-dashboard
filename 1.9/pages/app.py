...
# Check login
if "Customer Name" not in st.session_state or "role" not in st.session_state:
...
def generate_insights(df):
    ...
    for cat, sub in df.groupby("Category"):
        ...
        margin_score = min(max(margin, -0.2), 0.2) / 0.2   
        margin_score = (margin_score + 1) / 2              
...
with t4:
    st.subheader("Dự báo theo nhóm khách hàng (Segment) - 90 ngày tiếp theo")

    #Lấy danh sách Segment
    segments = df_filtered["Segment"].dropna().unique()

    #Dropdown Segment
    selected_segment = st.selectbox(
        "Chọn Segment muốn xem dự báo:",
        segments
    )
    ...
    df_seg = df_filtered[df_filtered["Segment"] == seg]

    #Gộp doanh thu theo tuần
    weekly_sales = df_seg.groupby(
    ...
    if len(weekly_sales) < 15:
        st.warning(f"Segment **{seg}** không đủ dữ liệu để dự báo.")
        st.stop()

    #Tạo feature thời gian
    weekly_sales["weekofyear"] = weekly_sales["ds"].dt.isocalendar().week
    weekly_sales["month"] = weekly_sales["ds"].dt.month
    weekly_sales["year"] = weekly_sales["ds"].dt.year
    weekly_sales["lag1"] = weekly_sales["y"].shift(1)
    weekly_sales = weekly_sales.dropna()

    #Chia train-test (4 tuần cuối)
    last_date = weekly_sales["ds"].max()
    test_start = last_date - pd.Timedelta(weeks=4)

    train = weekly_sales[weekly_sales["ds"] <= test_start]
    test = weekly_sales[weekly_sales["ds"] > test_start]

    features = ["weekofyear", "month", "year", "lag1"]
    ...
    #Dự báo tuần tiếp theo theo lag
    last_lag = weekly_sales["y"].iloc[-1]
    future_sales = []
    for i, row in future_df.iterrows():
        row_X = [[row["weekofyear"], row["month"], row["year"], last_lag]]
        pred = model_rf.predict(row_X)[0]
        future_sales.append(pred)
        last_lag = pred  

    future_df["y"] = future_sales

    #Vẽ biểu đồ
    plot_df = pd.concat([
        test[["ds","y"]].assign(Type="Actual").rename(columns={"ds":"Date","y":"Sales"}),
        future_df[["ds","y"]].assign(Type="Forecast").rename(columns={"ds":"Date","y":"Sales"})
    ])
    ...
    fig_seg = px.line(
        ...
        y="Sales",
        color="Type",
        title=f"Dự báo doanh thu 90 ngày cho Segment: {seg}",
        markers=True,
        template="plotly_white"
    )
    ...
    #Kết hợp actual + forecast 90 ngày
    forecast_90 = pd.concat([
        test[["ds","y"]].rename(columns={"ds":"Date","y":"Sales"}),
        future_df[["ds","y"]].rename(columns={"ds":"Date","y":"Sales"})
    ])

    #tổng doanh thu dự kiến
    total_forecast = forecast_90["Sales"].sum()

    #thay đổi trung bình theo tuần
    weekly_change = forecast_90["Sales"].pct_change().mean()
    ...
    #lời khuyên tổng thể
    advice = f"""
    **Lời khuyên tổng thể cho Segment {seg}:**  

    - Doanh thu dự kiến trong 90 ngày tới: **{total_forecast:,.0f}**  
    - Xu hướng chung: **{trend}**  
    - Biến động trung bình theo tuần: **{weekly_change*100:.2f}%**

    Gợi ý: 
    """
    if trend == "tăng":
        advice += "Có thể tăng cường marketing, chuẩn bị đủ hàng tồn kho để tận dụng đà tăng doanh thu."
    elif trend == "giảm":
        advice += "Cần xem xét giảm chi phí, khuyến mãi hoặc chiến dịch để kích cầu khách hàng."
    else:
        advice += "Doanh thu ổn định, duy trì chiến lược hiện tại, tối ưu chi phí và giữ mối quan hệ khách hàng."
